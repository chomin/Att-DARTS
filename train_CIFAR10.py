...
def main():
    ...
    parser.add_argument('--exp_path', type=Path, default=Path('exp'), help='experiment name')
    parser.add_argument('--seed', type=int, default=2, help='random seed')
    parser.add_argument('--arch', type=str, default='', help='which architecture to use')
    parser.add_argument('--arch_path', type=Path, help='which architecture of json to use')
    parser.add_argument('--grad_clip', type=float, default=5, help='gradient clipping')
    args = parser.parse_args()

    args.save = args.exp_path / f'CIFAR10-{time.strftime("%Y%m%d-%H%M%S")}'
    utils.create_exp_dir(args.save, scripts_to_save=glob.glob('*.py'))

    # ----- train -----
    my_dataset = MyDataset.CIFAR10
